...
def ManualSAMAdam(lr: float = 1e-3, rho=0.05, betas=(0.9, 0.999), eps=1e-8):
    return ManualTwolayerSAMAdam(lr=lr, rho=rho, betas=betas, eps=eps)

def ManualSAMAdaGrad(lr: float = 1e-2, rho=0.05, eps=1e-8):
    return ManualTwolayerSAMAdaGrad(lr=lr, rho=rho, eps=eps)
...
def _compute_grads(W1: torch.Tensor, W2: torch.Tensor, X: torch.Tensor, y: torch.Tensor):
    """
...
class ManualTwolayerSAMAdam(OptimizerState):
    ...
    def step(self, model, X, y, lr: float):
        ...
        with torch.no_grad():
            ...
            gnorm = torch.sqrt(g1.norm()**2 + g2.norm()**2 + 1e-16)
            scale = self.rho / (gnorm + 1e-16)
...
class ManualTwolayerSAMAdaGrad(OptimizerState):
